...
def managePlane():
    ...
    if request.method == "POST":
        ...
        if data: 
            ...
            return render_template("airSystemManage.html", noFound = (noFound, "airplane"))
        cursor.close()
        ...
        if data: 
            for each in data:
                print("data:", each)
            return render_template("airSystemManage.html", airplane = data)
        else: 
...
def manageAirport():
    if request.method == "POST":
        ...
        if data: 
            ...
            return render_template("airSystemManage.html", noFound = (noFound, "airport"))
        cursor.close()
        ...
        if data: 
            for each in data:
                print("data:", each)
            return render_template("airSystemManage.html", airport = data)
        else: 
...
def checkRatings(flight_num, dept_time):
    ...
    #fetch data
    cursor = conn.cursor()
    query = "SELECT airline_name,flight_num, dept_time, cust_email, rate, comments, AVG(rate) as avg_rate \
        FROM rates \
        WHERE (airline_name,flight_num, dept_time) = (%s, %s, %s) \
        GROUP BY airline_name,flight_num, dept_time"
    cursor.execute(query, (airline_name,flight_num, dept_time))
    data = cursor.fetchall()
    if data: 
        for each in data: 
...
def viewTopAgent():
    ...
    if request.method == 'POST': 
        ...
        if option == "by_sales_month": 
            ...
            WHERE agent_email IS NOT NULL AND DATE(purchase_time) BETWEEN NOW() - INTERVAL 30 DAY AND NOW() + INTERVAL 1 DAY \
            ...
            query = "SELECT  agent_email, COUNT(*) AS total_sales FROM ticket \
            WHERE agent_email IS NOT NULL AND DATE(purchase_time) BETWEEN NOW() - INTERVAL 1 YEAR AND NOW() + INTERVAL 1 DAY\
            GROUP BY agent_email ORDER BY total_sales DESC LIMIT 5"
            cursor.execute(query)
            data = cursor.fetchall()
            cursor.close()
...
def viewTopCustomer(): 
    ...
    AND DATE(purchase_time) BETWEEN NOW() - INTERVAL 1 YEAR AND NOW() + INTERVAL 1 DAY"
    cursor.execute(query,(airline_name))
    data1 = cursor.fetchall()
    max_times = 0
    for each in data1: 
    ...
    query2 = "SELECT cust_email, COUNT(*) AS travel_times FROM ticket WHERE airline_name = %s HAVING travel_times = %s"
...
def viewReport(message):
    ...
    if request == "default":
        ...
        query = "SELECT DATE(NOW()) - INTERVAL 1 MONTH AS curr_prev, DATE(NOW()) AS current,  COUNT(*) AS total_sales \
            FROM ticket WHERE date(purchase_time) between DATE(NOW()) - INTERVAL 1 MONTH AND DATE(NOW()) + INTERVAL 1 DAY and airline_name = %s"
        cursor.execute(query, (airline_name))
        info = cursor.fetchone()
        total_sales = info['total_sales'] #total sales
        from_date  = info['curr_prev'] # from_date
        to_date = info['curr'] # to_date
        default = "Default:"
        ...
        #access total_sales
        cursor = conn.cursor()
        query = "SELECT COUNT(*) as total_sales FROM ticket WHERE date(purchase_time) >= %s AND date(purchase_time) <= %s and airline_name = %s"
        ...
        if option == "sales_past_month":
            ...
            query = "SELECT DATE(NOW()) AS current, DATE(NOW()) - INTERVAL 1 YEAR AS curr_prev , COUNT(*) as total_sales \
                FROM ticket WHERE date(purchase_time) between DATE(NOW()) - INTERVAL 1 YEAR AND DATE(NOW()) + INTERVAL 1 DAY and airline_name = %s"
            cursor.execute(query, (airline_name))
            info = cursor.fetchone()
            print(info)
            total_sales = info['total_sales'] #total sales
    ...
    ORDER BY YEAR(purchase_time), MONTH(purchase_time) ASC"
    cursor.execute(query, (from_date, to_date, airline_name))
    ...
    for each in raw_data: 
        print(each)
        sales_dict["{}-{}".format(each["year"],each["month"])] = total_sales
    ...
    if request.method == "POST":
        ...
        if chosen == "by_3month": 
            ...
            query = "SELECT arr_at, city, count(*) as visit_time \
                    ...
                    WHERE S.arr_at  = airport.name AND DATE(purchase_time) BETWEEN NOW() - INTERVAL 3 MONTH and NOW() + INTERVAL 1 DAY\
                    GROUP BY arr_at ORDER BY visit_time DESC LIMIT 3"
        else: 
            title = "Top Three Destinations for the Past Year"
            query = "SELECT arr_at, city, count(*) as visit_time \
                    FROM ticket NATURAL JOIN flight as S, airport \
                    WHERE S.arr_at  = airport.name AND DATE(purchase_time) BETWEEN NOW() - INTERVAL 1 YEAR and NOW() + INTERVAL 1 DAY\
                    GROUP BY arr_at ORDER BY visit_time DESC LIMIT 3"
    else: 
